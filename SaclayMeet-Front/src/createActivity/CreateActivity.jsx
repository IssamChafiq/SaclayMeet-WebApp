...
// resize helper (keeps payload small)
async function fileToDataUrlResized(file, maxW = 1280, quality = 0.8) {
...
const CreateActivity = () => {
  ...
  const handleImageUpload = async (event) => {
    ...
    if (!file) return;
    try {
      const dataUrl = await fileToDataUrlResized(file, 1280, 0.8);
  ...
  const handleCreateActivity = async () => {
    ...
    if (!organizerId) {
      console.error("No organizerId in sessionStorage.");
    ...
    try {
      ...
      setTimeout(() => navigate("/viewActivities"), 1000);
    } catch (err) {
      console.error(err);
    ...
    <ThemeProvider theme={theme}>
      <div className="create-activity">
            ...
            <div className="form-container">
              <div className="form-section">
                <TextField label="Title" fullWidth variant="outlined"
                  placeholder="Tutoring help" value={title} onChange={(e) => setTitle(e.target.value)} />
                <TextField label="Place" fullWidth variant="outlined"
                  placeholder="Bat 620" value={location} onChange={(e) => setLocation(e.target.value)} />
                ...
                <TextField label="Event time start" fullWidth variant="outlined" type="time"
                  value={startTimeStr} onChange={(e) => setStartTimeStr(e.target.value)} />
                <TextField label="Event time end" fullWidth variant="outlined" type="time"
                  value={endTimeStr} onChange={(e) => setEndTimeStr(e.target.value)} />

                <TextField label="Capacity (optional)" fullWidth variant="outlined" type="number"
                  value={capacity} onChange={(e) => setCapacity(e.target.value)} />

                <TextField label="Description" variant="outlined"
                  placeholder="Please help with tutoring !" value={description}
        ...
        <div className="button-container">
          <Button variant="contained" color="salmon" className="create-button" onClick={handleCreateActivity}>
            Create activity
          </Button>
        </div>
      </div>
